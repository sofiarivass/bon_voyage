...
// Declaración de la clase Viaje (fuera del componente para evitar recrearla)
class Viaje {
...
function Inicio() {
    ...
    // Estado para controlar los inputs del formulario modal
    ...
    // Guardado automático en localStorage cuando cambia la lista de viajes
    useEffect(() => {
    ...
    const handleEditarViajeClick = () => {
        if (!viajeSeleccionado) return;
        setEditando(true);
        setImagenActual(viajeSeleccionado.imagen);
        setFormData({
            color: viajeSeleccionado.color,
            pais: viajeSeleccionado.pais,
            ciudad: viajeSeleccionado.ciudad,
            ida: viajeSeleccionado.ida,
            vuelta: viajeSeleccionado.vuelta,
            presupuesto: viajeSeleccionado.presupuesto,
            notas: viajeSeleccionado.notas
        });
    };
    ...
    const handleEliminarViaje = () => {
        ...
        setViajes(prev => prev.filter(v => v.id !== viajeSeleccionado.id));
        setViajeSeleccionado(null);

        // Cerrar modal de Bootstrap de forma segura en React
        const modalEl = document.getElementById('modal-detalles');
    ...
    // Capturar cambios en tiempo real de los inputs
    const handleInputChange = (e) => {
        const { id, value } = e.target;
        setFormData(prev => ({ ...prev, [id]: value }));
    };

    // Cargar imagen seleccionada en Base64
    ...
    const handleSubmit = (event) => {
        ...
        let fecha_ida = fechaLocal(formData.ida);
        let fecha_vuelta = fechaLocal(formData.vuelta);
        ...
        if (!editando) {
            ...
            setViajes(prev => prev.map(v => v.id === viajeSeleccionado.id ? viajeActualizado : v));
            setViajeSeleccionado(viajeActualizado);
        }

        // Cerrar modal de Formulario de forma segura en React
    ...
    // 1. Sincronizar estados según la fecha de hoy real
    const viajesConEstadosAlDia = viajes.map(v => {
        let fIda = fechaLocal(v.ida);
        let fVuelta = fechaLocal(v.vuelta);
        let est = "Completado";
        if (hoyActual < fIda) est = "Pendiente";
        else if (hoyActual >= fIda && hoyActual <= fVuelta) est = "En curso";
        return { ...v, estado: est };
    });

    // 2. Filtrar y ordenar
    const viajesFiltrados = viajesConEstadosAlDia.filter(v => {
        if (filtroSeleccionado === "todos") return true;
        return v.estado.toLowerCase() === filtroSeleccionado.replace("_", " ").toLowerCase();
    }).sort((a, b) => fechaLocal(a.ida) - fechaLocal(b.ida));

    // 3. Cálculos del Dashboard reactivos
    let planificados = 0;
    let completados = 0;
    let presupuestoTotal = 0;
    let diasTotales = 0;

    viajesConEstadosAlDia.forEach(v => {
        if (v.estado === "Pendiente" || v.estado === "En curso") {
            planificados++;
        } else if (v.estado === "Completado") {
            completados++;
        }
        presupuestoTotal += parseFloat(v.presupuesto) || 0;
        diasTotales += v.dias || 0;
    });
